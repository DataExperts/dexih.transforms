...
namespace dexih.functions.ml
{
    public class RecommendationAnalysis
    {
        ...
        public float RecommendationPredict(
            [TransformFunctionParameter(Name = "Model", Description = "The model generated by the recommendation training function." )] byte[] recommendationModel, 
            [TransformFunctionParameter(Description = "First item of a known pair." )]string item1, 
            [TransformFunctionParameter(Description = "Second item of a known pair." )]string item2)
        {
            ...
            {
                var mlContext = new MLContext();
                var model = Helpers.LoadModel(mlContext, recommendationModel, out _);
                _predictionEngine = mlContext.Model.CreatePredictionEngine<RecommendationEntry, RecommendationPrediction>(model);
            }
