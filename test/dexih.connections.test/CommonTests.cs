...
namespace dexih.connections.test
{
    public class CommonTests
    {
        ...
        public static void UnitTests(Connection connection, string databaseName, bool CanSort = true)
        {
            ...
            returnValue = connection.ExecuteInsert(table, new List<InsertQuery>() { insertQuery }).Result;
            ...
            var returnUpdate = connection.ExecuteUpdate(table, new List<UpdateQuery>() { updateQuery }).Result;
            ...
            //should return value2 from second row
            var returnDelete = connection.ExecuteDelete(table, new List<DeleteQuery>() { deleteQuery }).Result;
            ...
            var bulkResult = connection.ExecuteInsertBulk(table, testData).Result;
            ...
            var returnTransform = connection.GetTransformReader(table, null, null).Result;
            Transform reader = returnTransform.Value;
            ...
            returnTransform = connection.GetTransformReader(table, null, null).Result;
            reader = returnTransform.Value;
        ...
        public static void PerformanceTests(Connection connection, string databaseName)
        {
            ...
            for (int i = 0; i < 1000000; i++)
            {
                ...
                if(buffer >= 50000)
                {
                    ...
                    var bulkResult = connection.ExecuteInsertBulk(table, new ReaderMemory(table)).Result;
                    Assert.True(bulkResult.Success, "WriteDataBulk - Message:" + bulkResult.Message);
            ...
            for(int i = 0; i<100000; i++)
            {
                ...
                {
                     Filters = new List<Filter>() {  new Filter() { Column1 = "column0", CompareDataType = DataType.ETypeCode.String, Operator = Filter.ECompare.IsEqual, Value2 = "row-" + i.ToString() + " column-0"} },
                     Table = table.TableName,
                     UpdateColumns = new List<QueryColumn>() {  new QueryColumn("column1", DataType.ETypeCode.String, "updated") }
                });
            }

            var updateResult = connection.ExecuteUpdate(table, updateQueries).Result;
            ...
            {
                Columns = new List<SelectColumn>() { new SelectColumn("column1", SelectColumn.EAggregate.Count) },
                Filters = new List<Filter>() { new Filter() { Column1 = "column1", CompareDataType = DataType.ETypeCode.String, Operator = Filter.ECompare.IsEqual, Value2 = "updated" } },
                Rows = -1,
                Table = table.TableName
            };
            ...
            var deleteResult = connection.ExecuteDelete(table, deleteQueries).Result;
