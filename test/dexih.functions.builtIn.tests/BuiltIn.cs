...
//        [InlineData(typeof(BuiltIn.ConditionFunctions), nameof(ConditionFunctions.IsDaylightSavingTime), new object[] { "2015-09-24T21:22:48.2698750Z" }, false)]
        ...
        public void PreviousRowTests(Type type, string methodName, int count, object[] data, object[] expectedResult)
        {
            ...
            for (var a = 0; a < 2; a++) // run all tests twice to ensure reset functions are working
            {
                ...
                [Theory]
        [InlineData(typeof(CacheFunctions<>), nameof(CacheFunctions<int>.PreviousRowChange), 1, true, new object[] {null, 1, 2, null, 3, 4, 5}, new object[] {0, 1, 1, -2, 3, 1, 1})]
        [InlineData(typeof(CacheFunctions<>), nameof(CacheFunctions<int>.PreviousRowChange), 2, true, new object[] {null, 1, 2, null, 3, 4, 5}, new object[] {0, 0, 2, -1, 1, 4, 2})]
        [InlineData(typeof(CacheFunctions<>), nameof(CacheFunctions<int>.PreviousRowChange), 1, false, new object[] {null, 1, 2, null, 3, 4, 5}, new object[] {0, 1, 1, -2, 3, 1, 1})]
        [InlineData(typeof(CacheFunctions<>), nameof(CacheFunctions<int>.PreviousRowChange), 2, false, new object[] {null, 1, 2, null, 3, 4, 5}, new object[] {0, 0, 2, -1, 1, 4, 2})]
        public void PreviousRowChangeTests(Type type, string methodName, int count, bool noPreviousNull, object[] data, object[] expectedResult)
        {
            var function = Functions.GetFunction(type.FullName, methodName, BuiltInAssembly);
            var transformFunction = function.GetTransformFunction(typeof(double));
            ...
            for (var a = 0; a < 2; a++) // run all tests twice to ensure reset functions are working
            {
                for (var i = 0; i < data.Length; i++)
                {
                    var result = transformFunction.RunFunction(new[] { data[i], count, noPreviousNull }, CancellationToken.None);
                    Assert.NotNull(result.returnValue);
                    Assert.Equal(Convert.ToDouble(expectedResult[i]), Convert.ToDouble(result.returnValue));
                }
